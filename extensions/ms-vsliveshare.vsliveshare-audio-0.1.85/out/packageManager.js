...
class PackageManager {
    ...
    static getPackageCode(packageJSON, name) {
        if (packageJSON.runtimeDependencies) {
            ...
            const electronPackage = allPackages.find(pkg => pkg.code && pkg.code.indexOf(name) !== -1);
            if (electronPackage) {
                return electronPackage.code;
            }
        }
        return null;
    }
    ...
    unzipPackageAsync(pkg, baseUnpackPath) {
        return __awaiter(this, void 0, void 0, function* () {
            return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
                ...
                extractZip(pkg.tmpFile.name, { dir: baseUnpackPath }, function (err) {
                    tempProcess.noAsar = noAsar;
                    if (err) {
                        reject(new PackageError('Zip File Error:' + (err.message || ''), pkg, err));
                    }
                    else {
                        resolve();
                    }
