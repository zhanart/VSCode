...
function fetchCrateVersions(dependencies, shouldListPreRels, githubToken) {
    indicators_1.statusBarItem.setText("👀 Fetching crates.io");
    const responses = dependencies.map((item) => {
        return github_1.versions(item.key, githubToken)
            .then((json) => {
            return {
                item,
...
function parseAndDecorate(editor) {
    ...
    try {
        ...
        // Fetch Versions
        fetchCrateVersions(dependencies, !!shouldListPreRels, githubToken)
